<form action="<?= base_url(); ?>peoples/update_ahliwaris" method="post" enctype="multipart/form-data">
<div class="container">
    <div class="row mt-3">
      <div class="col-md-10">
  ...
  <div class="form-group">
    ...
    <select id="jk_aw" name="jk_aw" class="form-control">
      <?php foreach ($jk as $j) : ?>
        <option value="<?= $j['id']; ?>" <?php if($ahli_waris['jk_aw'] == $j['id']){ echo "selected";} ?>>
          <?= $j['jk']; ?>
        </option>
      <?php endforeach; ?>
    </select>
  ...
  <div class="form-group">
    ...
    <!-- <input type="text" class="form-control" id="stts_kel" name="stts_kel" value="<?= $ahli_waris['stts_kel']; ?>">
    <small>1 = Suami/Istri | 2 = Anak | 3 = Orang Tua | 4 = Pihak Yang Ditunjuk</small> -->

    <select id="stts_kel" name="stts_kel" class="form-control">
      <?php foreach ($status_keluarga as $sk) : ?>
        <option value="<?= $sk['id']; ?>" <?php if($ahli_waris['stts_kel'] == $sk['id']){ echo "selected";} ?>>
          <?= $sk['stts_kel']; ?>
        </option>
      <?php endforeach; ?>
    </select>
  ...
  <div class="form-group">
    ...
    <!-- <input type="text" class="form-control" id="stts_tanggn" name="stts_tanggn" value="<?= $ahli_waris['stts_tanggn']; ?>">
    <small>MT = Menjadi Tanggungan | TMT = Tidak Menjadi Tanggungan</small> -->

    <select id="stts_tanggn" name="stts_tanggn" class="form-control">
      <?php foreach ($status_tanggungan as $st) : ?>
        <option value="<?= $st['id']; ?>" <?php if($ahli_waris['stts_tanggn'] == $st['id']){ echo "selected";} ?>>
          <?= $st['stts_tanggn']; ?>
        </option>
      <?php endforeach; ?>
    </select>
            ...
            <tr>
              <!-- <th>
                <?php  
                          ...
                          <iframe src="<?= base_url('assets/img/kk/').$alldata['scan_kk'];  ?>" width="100%" height="300px">
                    </iframe>
                          <?php else : ?>
                <img class="img-thumbnail" src="<?= base_url('assets/img/kk/').$alldata['scan_kk']; ?>" width="200" height="100" data-toggle="modal" data-target="#fullimage">
                <?php endif; ?>
              </th> -->
